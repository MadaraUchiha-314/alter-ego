...
| [release-automation](./release-automation.md) | semantic-release on merge to `main`: derives the version bump from Conventional Commits, stamps `plugin.json`, updates the changelog, tags and cuts the GitHub release, with a blocking commitlint check on PR titles — the source of the versions `alter-ego:upgrade` walks |
